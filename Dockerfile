...
COPY --from=build --chown=node:node /app/scripts/cosmetics-import.ts ./scripts/cosmetics-import.ts
COPY --from=build --chown=node:node /app/cosmetics/catalogue ./cosmetics/catalogue
COPY --from=build --chown=node:node /app/next.config.ts ./next.config.ts
COPY --from=build --chown=node:node /app/tsconfig.json ./tsconfig.json
COPY --from=build --chown=node:node /app/package.json ./package.json
USER node
EXPOSE 3000
CMD ["sh", "-c", "node --import tsx server/db/migrate.ts && node --import tsx scripts/cosmetics-import.ts cosmetics/catalogue && exec node --import tsx server/index.ts"]
