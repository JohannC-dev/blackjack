...
/** The profile opens as its own full-screen page over the club. */
  ...
  return (
    <Dialog open={!!playerId} onOpenChange={(open) => !open && onClose()}>
      <DialogContent
        ...
        className="fixed inset-0 top-0 left-0 z-50 flex h-dvh max-h-none w-screen max-w-none translate-x-0 translate-y-0 flex-col gap-0 overflow-y-auto overscroll-contain rounded-none border-0 bg-minuit-bg p-0 font-sans text-foreground shadow-none data-[state=closed]:zoom-out-100 data-[state=open]:zoom-in-100 sm:max-w-none"
      >
        <ScreenBar title={profile?.name} />
        {profile ? (
...
/** The bar that gets the player back to the table they came from. */
function ScreenBar({ title }: { title?: string }) {
  return (
    <div className="sticky top-0 z-10 border-b border-white/[0.06] bg-minuit-bg/90 backdrop-blur">
      <div className="mx-auto flex w-full max-w-5xl items-center gap-3 px-4 py-3 sm:px-8">
        ...
        <span className="min-w-0 flex-1 truncate text-center text-xs tracking-[0.22em] text-muted-foreground uppercase">
    ...
    <>
      <div className="relative overflow-hidden border-b border-white/[0.06]">
        <div
          ...
          aria-hidden="true"
        />
        <div className="relative mx-auto w-full max-w-5xl px-5 pt-8 pb-7 sm:px-8 sm:pt-10">
      ...
      <div className="mx-auto w-full max-w-5xl flex-1 px-5 pt-6 pb-16 sm:px-8">
