...
socket.on("emote", (e) =>
  console.log("received", e.emote, e.fromName, "->", e.targetId?.slice(0, 6)),
);
socket.on("connect", () => {
  socket.emit(
    "join",
    {
      profile: { token: randomUUID(), name: "Bot Jojo", balance: 5000 },
      tableId: "MINUIT",
    },
    (ack: any) => {
      me = ack.playerId;
      ...
      if (game === "blackjack")
        socket.emit("blackjack:join", (a: any) => console.log("bj", a));
      else
        socket.emit(
          "poker:command",
          { type: "match", mode: "cash", stake: 20 },
          (a: any) => console.log("poker", a),
        );
    },
  );
});
